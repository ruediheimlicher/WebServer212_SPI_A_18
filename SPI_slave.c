...
extern volatile uint8_t spi_startdaten;				// Startdaten des Slave, bestimmen Art der Daten
extern volatile uint8_t spibuffer[SPI_BUFSIZE];		// Der SPI-buffer, der vom Master ausgelesen werden kann.
extern volatile uint8_t spi_enddaten;					// Enddaten des Slave, Quittung fuer vollstaendige Uebertragung
...
volatile uint8_t								spi_startdaten;
volatile uint8_t								spibuffer[SPI_BUFSIZE];
volatile uint8_t								spi_enddaten;
...
#define OSZITOG OSZIPORT ^= (1<<PULS)
...
#define SPI_PULSE_DELAY			70 // delay in ns
...
ISR( INT0_vect )
{
	...
	if (spistatus & (1<<ACTIVE_BIT))									// CS ist LO, Interrupt ist OK
	{
		...
		if (spistatus & (1<<STARTDATEN_BIT))						// spi_startdaten senden, in_startdaten laden
		{
		...
		//	LB
		else if (spistatus & (1<<LB_BIT))					// spi_lbdaten senden, in_lbdaten laden
		{
			...
			// Output laden
			if (spi_lbdaten & (1<<(7-bitpos)))						// bit ist HI
			{
				SPI_CONTROL_PORT |= (1<<SPI_CONTROL_MISO);
			}
			else																// bit ist LO
		...
		//	HB
		else if (spistatus & (1<<HB_BIT))					// spi_hbdaten senden, in_hbdaten laden
		{
			...
			// Output laden
			if (spi_hbdaten & (1<<(7-bitpos)))						// bit ist HI
			{
				SPI_CONTROL_PORT |= (1<<SPI_CONTROL_MISO);
			}
			else																// bit ist LO
		...
		else if (spistatus & (1<<ENDDATEN_BIT))					// spi_enddaten senden, in_enddaten laden
		{
			...
			// Output laden
//			if (spi_enddaten & (1<<(7-bitpos)))						// bit ist HI
			...
			if (bitpos>=8)	// Byte fertig
			{
				...
				bitpos=0;
				if (spi_startdaten + in_enddaten==0xFF)
				{
					//lcd_putc('+');
				...
				// 24.6.2010
//				spi_startdaten=0xC0; ergab nicht korrekte Pruefsumme mit in_enddaten
...
//				spi_lbdaten=0;
		...
		else			// Datenarray in inbuffer laden, Daten von spibuffer senden
		
		{
			...
			// Output laden
			if (spibuffer[ByteCounter] & (1<<(7-bitpos)))		// bit ist HI
			{
				SPI_CONTROL_PORT |= (1<<SPI_CONTROL_MISO);
			}
			else																// bit ist LO
		...
		}						//  Datenarray in inbuffer laden, Daten von spibuffer senden
